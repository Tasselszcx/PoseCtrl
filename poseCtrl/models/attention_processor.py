...
class PoseAttnProcessorV1(nn.Module):
    r"""
    Attention processor for PoseAttnProcessor.
    Args:
        hidden_size (`int`):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            ...
            encoder_hidden_states, point_hidden_states, feature_hidden_states = (
                encoder_hidden_states[:, :end_pos_point, :],
                encoder_hidden_states[:, end_pos_point:end_pos, :],
                encoder_hidden_states[:, end_pos:, :]
            )
            if attn.norm_cross:
                encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
        ...
        # for point
        point_key = self.to_k_pose(point_hidden_states)
        point_value = self.to_v_pose(point_hidden_states)

        point_key = attn.head_to_batch_dim(point_key)
        point_value = attn.head_to_batch_dim(point_value)
        ...
        hidden_states = hidden_states + self.scale * point_hidden_states + self.scale * feature_hidden_states
...
class PoseAttnProcessorV2IP(nn.Module):
    r"""
    Attention processor for PoseAttnProcessor.
    Args:
        hidden_size (`int`):
...
class PoseAttnProcessorV2Ctrl(nn.Module):
    r"""
    Attention processor for PoseAttnProcessor.
    Args:
        hidden_size (`int`):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, scale=1.0, num_tokens=8):
        super().__init__()
...
class PoseAttnProcessorV4(nn.Module):
    r"""
    Attention processor for PoseAttnProcessor.
    Args:
        hidden_size (`int`):
