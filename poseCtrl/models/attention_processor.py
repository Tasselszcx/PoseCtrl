...
class PoseAttnProcessorV7(nn.Module):
    r"""
    Attention processor for PoseAttnProcessor.
    Args:
        hidden_size (`int`):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, scale=1.0, num_point_tokens=77):
        super().__init__()
        ...
        self.to_k_pose = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.to_v_pose = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.batch_to_head_dim = nn.Linear(hidden_size, hidden_size)
        self.to_point = nn.Linear(hidden_size *2, hidden_size)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        hidden_states = torch.bmm(attention_probs, value)
        hidden_states = attn.batch_to_head_dim(hidden_states)
        hidden_states_background = self.batch_to_head_dim(hidden_states)

        point_hidden_states = torch.cat([hidden_states_background, point_hidden_states], dim=-1)
        point_hidden_states = self.to_point(point_hidden_states)
